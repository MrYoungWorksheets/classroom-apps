...
(function () {
  ...
  const creditsBefore9999 = game.player.credits;
  liveEvents = [];
  handleDevCode('9999');
  assert(game.player.credits === creditsBefore9999 + 30000, '9999 should add exactly 30,000 credits');
  assert(game.stats.mathMissionsCompleted === completedBefore && !game.missionLocked, '9999 should not complete or lock mission');
  assert(game.ui.lastSectorActionResult?.message === 'Dev credits granted: +30,000 credits.', '9999 should show an action result');
  assert(liveEvents[0]?.message === 'Dev credits granted: +30,000 credits.', '9999 should push live event feedback');
  assert(game.log.some((entry) => entry === 'Dev credits granted: +30,000 credits.'), '9999 should log feedback');

  game = defaultGameState();
  game.player.currentSector = 2;
  game.player.cargo.Ore = 10;
  game.player.cargo.Food = 1;
  game.player.cargo.Smuggled = 4;
  game.player.cargoCostBasis.Ore = { quantity: 10, totalCost: 100, known: true };
  game.player.cargoCostBasis.Food = { quantity: 1, totalCost: 8, known: true };
  const creditsBefore6767 = game.player.credits;
  const cargoBefore6767 = { ...game.player.cargo };
  const remainingBefore6767 = cargoSpaceLeft();
  liveEvents = [];
  handleDevCode('6767');
  const oreAdded6767 = game.player.cargo.Ore - cargoBefore6767.Ore;
  const foodAdded6767 = game.player.cargo.Food - cargoBefore6767.Food;
  const techAdded6767 = game.player.cargo.Tech - cargoBefore6767.Tech;
  assert(oreAdded6767 + foodAdded6767 + techAdded6767 === remainingBefore6767, '6767 should fill all remaining cargo space');
  assert(Math.max(oreAdded6767, foodAdded6767, techAdded6767) - Math.min(oreAdded6767, foodAdded6767, techAdded6767) <= 1, '6767 should balance added Ore/Food/Tech');
  assert(game.player.credits === creditsBefore6767, '6767 should cost no credits');
  assert(cargoUsed() === game.player.cargoCapacity, '6767 should respect cargo capacity');
  assert(game.player.cargoCostBasis.Ore.totalCost === 100 && game.player.cargoCostBasis.Tech.totalCost === 0 && game.player.cargoCostBasis.Tech.known, '6767 should update free cargo cost basis safely');
  assert(game.ui.lastSectorActionResult?.message === 'Dev cargo loaded: balanced Ore/Food/Tech added at no cost.', '6767 should show an action result');
  assert(liveEvents[0]?.message === 'Dev cargo loaded: balanced Ore/Food/Tech added at no cost.', '6767 should push live event feedback');
  assert(game.log.some((entry) => entry === 'Dev cargo loaded: balanced Ore/Food/Tech added at no cost.'), '6767 should log feedback');

  game = defaultGameState();
  game.player.shipId = 'atlasHauler';
  game.player.ownedShips.push('atlasHauler');
  game.player.upgrades.engine = 4;
  game.player.upgrades.shield = 6;
  game.player.fighterCapacity = 1;
  game.player.maxHull = 1;
  game.player.maxFuel = 1;
  game.player.fighters = 0;
  game.player.hull = 3;
  game.player.fuel = 2;
  const creditsBefore8888 = game.player.credits;
  const shipBefore8888 = game.player.shipId;
  const expectedFighterCapacity8888 = calculateFighterCapacity(currentShip(), game.player.upgrades);
  const expectedMaxHull8888 = currentShip().maxHull + Math.max(0, (game.player.upgrades.shield || 1) - 1) * 4;
  const expectedMaxFuel8888 = calculateFuelCapacity(currentShip(), game.player.upgrades);
  liveEvents = [];
  applyDevCode('8888');
  assert(game.player.shipId === shipBefore8888, '8888 should not change ship type');
  assert(game.player.credits === creditsBefore8888, '8888 should not change credits');
  assert(game.player.fighterCapacity === expectedFighterCapacity8888 && game.player.fighters === expectedFighterCapacity8888, '8888 should fill fighters to current fighter capacity');
  assert(game.player.maxHull === expectedMaxHull8888 && game.player.hull === expectedMaxHull8888, '8888 should repair hull to current max hull');
  assert(game.player.maxFuel === expectedMaxFuel8888 && game.player.fuel === expectedMaxFuel8888, '8888 should fill fuel to current max fuel');
  assert(game.ui.lastSectorActionResult?.message === 'Combat readiness maximized: fighters, hull, and fuel restored.', '8888 should show an action result');
  assert(liveEvents[0]?.message === 'Combat readiness maximized: fighters, hull, and fuel restored.', '8888 should push live event feedback');
  assert(game.log.some((entry) => entry === 'Combat readiness maximized: fighters, hull, and fuel restored.'), '8888 should log feedback');
  ...
  game.player.cargo.Tech = 100;
  game.planets[claimed.id].stored.Tech = 35;
  liveEvents = [];
  panels.docked.innerHTML = renderScreenContent('planets');
  assert(panels.docked.innerHTML.includes('Deposit All') && panels.docked.innerHTML.includes('Deposit 1') && panels.docked.innerHTML.includes('Deposit 10'), 'planet management shows Deposit All while keeping Deposit 1 and Deposit 10 cargo controls');
  depositToPlanet('Tech', 'max');
  assert(game.player.cargo.Tech === 0, 'Deposit All removes all carried Tech from ship cargo');
  assert(game.planets[claimed.id].stored.Tech === 135, 'Deposit All adds all carried Tech to planet storage');
  assert(game.log.some((entry) => entry.includes('Deposited 100 Tech to ' + claimed.name)), 'Deposit All records a Captain Log entry');
  assert(game.ui.lastSectorActionResult?.message.includes('Deposited 100 Tech to ' + claimed.name), 'Deposit All reports through immediate action result');
  assert(liveEvents[0]?.message.includes('Deposited 100 Tech to ' + claimed.name), 'Deposit All reports through live events');
  panels.docked.innerHTML = renderScreenContent('planets');
  assert(panels.docked.innerHTML.includes('None aboard'), 'planet management explains when no cargo is aboard for Deposit All');
  const storedOreBeforeDepositOne = game.planets[claimed.id].stored.Ore;
  game.player.cargo.Ore = 3;
  depositToPlanet('Ore', 1);
  assert(game.player.cargo.Ore === 2 && game.planets[claimed.id].stored.Ore === storedOreBeforeDepositOne + 1, 'existing Deposit 1 behavior still transfers one cargo unit');
  game.player.cargo.Food = 0;
  ...
  assert(panels.docked.innerHTML.includes('Delivery / Fetch Contracts — accepted contracts') && panels.docked.innerHTML.includes('Bounty Board — automatic pirate bounties') && panels.docked.innerHTML.includes('Math Contracts — training / reward contracts'), 'mission terminal explains contract terminology');
  ...
  assert(panels.docked.innerHTML.includes('Bounties are automatic') && panels.docked.innerHTML.includes('No acceptance is required') && panels.docked.innerHTML.includes('reward is paid after combat'), 'bounty board explains automatic combat-paid bounties');
  assert(panels.docked.innerHTML.includes('data-plot-bounty') || panels.docked.innerHTML.includes('Route unknown. Explore nearby lanes first.'), 'bounty cards include route or unknown-route action');
  const bountyHtmlAtSector = (() => {
    const previousSector = game.player.currentSector;
    game.player.currentSector = 13;
    const html = renderBountyBoardContent();
    game.player.currentSector = previousSector;
    return html;
  })();
  assert(bountyHtmlAtSector.includes('Target here. Open Combat.') && bountyHtmlAtSector.includes('data-open-bounty-combat="13"'), 'bounty card exposes open combat when target is in current sector');
  game.revealedSectors = [1, 2, 3, 11, 12, 13];
  openScreen('specialMissions');
  liveEvents = [];
  plotBountyRoute(13);
  assert(game.ui.warpDestination === 13 && game.ui.lastSectorActionResult?.message.includes('Bounty route set: Sector 13. Next jump: Sector 2.'), 'bounty route button sets warp destination and immediate action result');
  assert(game.ui.activeScreen === 'cockpit', 'successful bounty route from mission terminal returns to cockpit');
  assert(game.ui.lastSectorActionResult?.message.includes('Autopilot is ready in the cockpit'), 'route return result tells the player autopilot is ready in the cockpit');
  assert(panels.sector.innerHTML.includes('<details class="warp-panel collapsible-system compact-section" open>'), 'cockpit warp/autopilot route summary is visible after route return');
  assert(liveEvents.some((event) => event.title.includes('Bounty Route Set') && event.message.includes('Sector 13')), 'bounty route setting pushes a live event');
  game = defaultGameState();
  game.player.currentSector = 11;
  game.visitedSectors = [11];
  game.revealedSectors = [11];
  game.player.upgrades.scanner = 2;
  const scoutBountyHtml = renderBountyBoardContent();
  assert(scoutBountyHtml.includes('Scout Toward Target') && scoutBountyHtml.includes('Target Intel'), 'bounty board renders scout action and target intel for visible unexplored targets');
  openScreen('specialMissions');
  plotBountyRoute(13);
  assert(game.ui.warpDestination === 13 && game.ui.exploratoryWarp === true && game.ui.lastSectorActionResult?.message.includes('Next jump: Sector 12'), 'bounty scout action sets exploratory warp destination');
  assert(game.ui.activeScreen === 'cockpit', 'successful scout route from bounty board returns to cockpit');
  game = defaultGameState();
  game.player.currentSector = 1;
  game.player.upgrades.scanner = 1;
  const beforeUnknownVisited = game.visitedSectors.join(',');
  const beforeUnknownRevealed = game.revealedSectors.join(',');
  openScreen('specialMissions');
  plotBountyRoute(13);
  assert(game.player.currentSector === 1 && game.ui.warpDestination !== 13 && game.visitedSectors.join(',') === beforeUnknownVisited && game.revealedSectors.join(',') === beforeUnknownRevealed, 'unknown bounty routes do not reveal hidden lanes or travel instantly');
  assert(game.ui.activeScreen === 'specialMissions', 'failed route setting stays on the current terminal screen');
  ...
  game.revealedSectors = [1, 2, 3, 11, 12, 13, 18, 19, 23, 24];
  openScreen('specialMissions');
  plotDeliveryRoute(delivery.id);
  assert(game.ui.warpDestination === delivery.targetSector, 'delivery route button sets warp destination');
  assert(game.ui.activeScreen === 'cockpit', 'successful delivery/fetch route from mission terminal returns to cockpit');
  assert(game.ui.lastSectorActionResult?.message.includes('Autopilot is ready in the cockpit'), 'delivery route return shows immediate cockpit-ready feedback');
